...
def stretchImage(data, s=0.003, bins=2000):  # 线性拉伸，去掉最大最小0.5%的像素值，然后线性拉伸至[0,1]
...
def zmIce(I, ratio=2, radius=3):  # 常规的ACE实现
    para = getPara(radius)
...
def ACE(I, ratio=2, radius=3):  # rgb三通道分别增强，ratio是对比度增强因子，radius是卷积模板半径
    res = np.zeros(I.shape)
    for k in range(3):
